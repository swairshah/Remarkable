#!/bin/bash
# Launch collab in full-takeover mode on the reMarkable 2: stop xochitl,
# host the e-ink engine with the bundled rm2fb server (timower/rM2-stuff),
# run collab against it (instant ink), ALWAYS restore xochitl on exit.
#
# Exit collab: 5-finger tap or SIGTERM; the power button sleeps instead.
# Escape hatch if anything wedges: ssh root@<tablet> 'systemctl start xochitl'.
...
HOME=/home/root PATH="/home/root/bin:/home/root/opt/node/bin:/usr/bin:/bin:/usr/sbin:/sbin" \
    "$HERE/collab" >>/tmp/collab.log 2>&1
echo "collab-takeover: closed ($?), restoring xochitl"
