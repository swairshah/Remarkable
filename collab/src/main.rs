//! collab — handwrite to a background pi agent, watch its replies stream
//! back, on the reMarkable 2.
//!
//! It follows pi-collab's product exactly, on the riddle app's display/input
//! stack. The screen is three bands:
...
//! scrolled away.
//!
//! Two display backends, picked at runtime (display.rs): windowed under
//! AppLoad when QTFB_KEY is set, or FULL TAKEOVER against a running rm2fb
//! server — xochitl stopped, the vendor e-ink engine driven directly, pen
//! strokes on the DU waveform. Takeover is the low-latency mode and the
//! reason this app exists; scripts/takeover.sh owns the xochitl dance.
//! In takeover we also own the touch panel (scroll, taps, 5-finger quit)
//! and the power button (sleep page + suspend + wifi heal).
//!
//! Module map:
//!   fb.rs      the shared RGB565 framebuffer (either backend's shm)
//!   display.rs backend abstraction: per-update waveform intent
//!   qtfb.rs    AppLoad protocol (windowed backend + its input events)
//!   rm2fb.rs   rm2fb server client (takeover backend)
//!   pen.rs     direct Wacom digitizer input
//!   touch.rs   raw multitouch (takeover only)
//!   power.rs   power button + suspend + wifi heal (takeover only)
 ...
 * the leftmost column) — traced from the pi.dev icon */
const LOGO: [u8; 8] = [
    0b11111100,
    0b11111100,
    0b11001100,
...
/* Takeover exit, xochitl-style: swipe down from the top edge to reveal a
 * CLOSE button, tap it to leave. (Windowed mode gets xochitl's own chrome;
 * a 5-finger tap still works as an undocumented fallback.) */
const EDGE_Y: i32 = 16; /* a swipe must start this close to the bezel */
const SWIPE_DIST: i32 = 90; /* and travel this far down to count */
const CLOSE_X0: i32 = 16;
const CLOSE_Y0: i32 = 12;
const CLOSE_BTN_W: i32 = 190;
const CLOSE_BTN_H: i32 = 64;
const CLOSE_TTL: Duration = Duration::from_secs(4); /* auto-hide */
 ...
 * viewport is a big region, so its redraws are throttled harder. rm2fb
 * updates are synchronous and cheap, so takeover coalesces less. */
...
struct App {
    ...
    ink_flush: Duration, /* per-backend coalescing window for pen strokes */
    takeover: bool,
    swipe_from: Option<i32>, /* a top-edge swipe in progress (its start y) */
    close_until: Option<Instant>, /* CLOSE button is showing; auto-hide at */
...
impl App {
    ...
    fn draw_header(&mut self) {
        ...
        /* keep the CLOSE button on top of routine header repaints (the
         * streaming dot etc.) while it's showing */
        if self.close_until.is_some() {
            self.paint_close_button();
        }
        self.fb.fill_rect(0, HEADER_H - 2, FB_W, 2, BLACK);
        self.disp.update(0, 0, FB_W, HEADER_H, Wave::Ink);
    }
    ...
    /// `clear_canvas` is set, a blank writing area.
    fn draw_input_strip(&mut self, clear_canvas: bool) {
        ...
        self.disp.update(0, INPUT_Y0, FB_W, INPUT_H, Wave::Ink);
    }
    ...
    fn set_nib(&mut self, i: usize) {
        if i != self.nib {
            ...
            self.paint_nib_buttons();
            self.disp
                .update(NIB0_X, NIB_Y, nib_x(2) + NIB_W - NIB0_X, NIB_H, Wave::Ink);
        }
    }
    ...
    fn redraw_view(&mut self) {
         ...
         * one we ink with */
        self.redraw_view_wave(Wave::Text);
    }

    /// Repaint the viewport with a specific waveform intent. Scrolling uses
    /// the fast flash-free one; streaming/settled text uses the quality one
    /// so the antialiasing is clean.
    fn redraw_view_wave(&mut self, wave: Wave) {
        conv::draw(&mut self.fb, &self.entries, VIEW_Y0, VIEW_Y1, self.scroll, self.pi_px);
        self.disp.update(0, VIEW_Y0, FB_W, VIEW_Y1 - VIEW_Y0, wave);
    ...
    /// The takeover exit affordance: an inverted CLOSE button in the top-left
    /// corner, revealed by a top-edge swipe (like xochitl's app chrome).
    ...
    fn paint_close_button(&mut self) {
    ...
    /// Hide the CLOSE button (tap elsewhere or timeout): the header repaint
    /// covers it.
    ...
    /// Full-panel refresh: repaint from the current framebuffer with the
    /// deghosting waveform. Clears accumulated partial-update residue.
    ...
    /* -- finger: scroll + buttons (from qtfb events or the raw panel) -- */
    ...
    fn touch(&mut self, phase: Phase, x: i32, y: i32) {
        ...
        match phase {
            Phase::Press => {
                /* the CLOSE button, when showing, takes the whole tap: hit
                 * it = exit, anywhere else = dismiss (and nothing under the
                 * finger fires, so dismissing can't press REFRESH) */
                if self.close_until.is_some() {
                    if in_rect(x, y, CLOSE_X0, CLOSE_Y0, CLOSE_BTN_W, CLOSE_BTN_H) {
                        println!("collab: close button");
                        RUNNING.store(false, Ordering::Relaxed);
                    } else {
                ...
                /* takeover: a touch starting at the very top edge begins the
                 * reveal swipe, not a button press (buttons start lower) */
            ...
            Phase::Release => {
                ...
                if self.drag_last.take().is_some() && self.scroll_pending {
                    ...
                    self.redraw_view_wave(Wave::Ink);
                }
            }
        }
    }
...
fn sleep_cycle(
    ...
    /* Suspend, and confirm via the kernel's success counter: the EPD can
     * refuse to sleep while its post-update discharge timer runs (the whole
     * suspend aborts), so retry until it sticks. */
    ...
    'sleeping: loop {
        ...
        if attempts >= 8 {
            println!("collab: suspend never happened ({attempts} tries); waking the page");
            break;
        }
...
fn main() -> std::process::ExitCode {
    ...
    let pi = match Pi::spawn() {
        Ok(p) => Some(p),
        Err(e) => {
            eprintln!("collab: could not start pi: {e}");
            None
        }
    };
     ...
     * harness). collab is meant to run fullscreen. */
    ...
    /* takeover: touch and the power button are ours too */
    let mut touchdev = if takeover {
        touch::TouchDevice::open()
            .map_err(|e| eprintln!("collab: no touch device ({e}) — scroll/taps disabled"))
            .ok()
    } else {
        None
    };
    let mut powerdev = if takeover {
        power::PowerButton::open()
            .map_err(|e| eprintln!("collab: no power button ({e})"))
            .ok()
    } else {
        None
    };
    /* ignore power presses briefly after a wake: the waking press itself
     * (and key bounce) arrives on our grabbed fd and must not re-suspend */
     ...
     * COLLAB_AUTO_SLEEP_MIN (minutes), 0 disables. */
    let auto_sleep_min: u64 = std::env::var("COLLAB_AUTO_SLEEP_MIN")
        .ok()
    ...
    if takeover {
        app.entries
            .push(Entry::Note("swipe down from the top edge to exit".into()));
    }
    ...
    while RUNNING.load(Ordering::Relaxed) {
        /* poll the display socket (windowed), the pen hardware, pi's stdout,
         * and (takeover) the touch panel + power button; the timeout is
         * whichever pending flush comes due first. poll() ignores fd -1. */
        ...
        let mut pfds = [
            ...
            libc::pollfd {
                fd: touchdev.as_ref().map_or(-1, |t| t.raw_fd()),
                events: libc::POLLIN,
                revents: 0,
            },
            libc::pollfd {
                fd: powerdev.as_ref().map_or(-1, |p| p.raw_fd()),
                events: libc::POLLIN,
                revents: 0,
            },
        ];
        ...
        /* -- power button (takeover): sleep page + suspend + restore -- */
        ...
        /* -- raw touch panel (takeover): scroll, taps, 5-finger quit -- */
        if pfds[3].revents & libc::POLLIN != 0 {
            if let Some(t) = touchdev.as_mut() {
                let (evs, quit) = t.drain();
                if quit {
                    println!("collab: 5-finger quit");
                    RUNNING.store(false, Ordering::Relaxed);
                }
                if !evs.is_empty() {
                    last_activity = Instant::now();
        ...
        /* -- qtfb socket (windowed): touch, AppLoad pen fallback, window
         * lifecycle -- */
            ...
            while let Some(event) = app.disp.try_next_event() {
                match event {
                    ...
                    Event::Pen { phase, x, y, .. } => {
                         ...
                         * Only when there's no digitizer at all (the preview
                         * harness) do we ink from AppLoad. */
                        if !direct_pen {
                            let ph = match phase {
        ...
        if app.ink_dirty.is_some() && app.last_ink_flush.elapsed() >= app.ink_flush {
            ...
            /* pen ink wants the low-latency waveform */
            app.disp
                .update(x0.max(0), y0.max(0), x1 - x0 + 1, y1 - y0 + 1, Wave::Ink);
        ...
        /* auto-hide an untapped CLOSE button */
        ...
        if let (Some(limit), Some(p)) = (auto_sleep, powerdev.as_mut()) {
            /* deferred while pi is mid-turn (streaming clears on End, so
             * this can't hold the device awake forever) */
            if !app.streaming
                && last_activity.elapsed() >= limit
                && Instant::now() >= power_grace
            {
                println!("collab: idle {auto_sleep_min}min -> auto-sleep");
                sleep_cycle(&mut app, p, &mut pen, &mut touchdev);
                power_grace = Instant::now() + Duration::from_secs(3);
                last_activity = Instant::now();
            }
