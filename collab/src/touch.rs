...
//! COLLAB_TOUCH_FLIP=none|x|y|xy in case another panel revision differs.
//!
...
impl TouchDevice {
    ...
    pub fn open() -> io::Result<Self> {
        for i in 0..8 {
            ...
            let flip = std::env::var("COLLAB_TOUCH_FLIP").unwrap_or_else(|_| "y".into());
            let (flip_x, flip_y) = (flip.contains('x'), flip.contains('y'));
            println!(
                "collab: touch /dev/input/event{i} ({}x{}, flip {})",
