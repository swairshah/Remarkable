#!/usr/bin/env python3
"""Fake AppLoad/qtfb server for collab's `make preview` (no tablet).
...
(test/fake-pi.py) wired in via COLLAB_BIN, scripts some handwriting +
...
def main():
    ...
    env = dict(os.environ,
               QTFB_KEY="12345",
               COLLAB_BIN=os.path.join(here, "fake-pi.py"),
               COLLAB_HISTORY="/src/build/hist.jsonl",  # persists on host
               HOME="/tmp")
