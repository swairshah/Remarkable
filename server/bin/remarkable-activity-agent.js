...
function parseArgs(argv) {
  const cli = {
    ...
    sourceDir: import_node_path.default.join(import_node_os.homedir(), "remarkable-backup", "xochitl"),
    stateDir: import_node_path.default.join(import_node_os.homedir(), "remarkable-exports", "activity-agent"),
    outputHtml: import_node_path.default.join(import_node_os.homedir(), "notes", "updates", "index.html"),
    envFile: import_node_path.default.join(import_node_os.homedir(), ".env"),
    rerender: false
  };
...
async function main() {
  ...
  if (cli.rerender) {
    const summary2 = await exists(latestPath) ? (await import_node_fs.promises.readFile(latestPath, "utf8")).trim() : "(no summary published yet)";
    const history2 = await readHistory(historyPath);
    const changes2 = history2[0]?.changes ?? [];
    await import_node_fs.promises.mkdir(import_node_path.default.dirname(cli.outputHtml), { recursive: true });
    await import_node_fs.promises.writeFile(cli.outputHtml, renderHtml(summary2, changes2, history2), "utf8");
