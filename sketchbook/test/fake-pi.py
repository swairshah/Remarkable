...
Speaks just enough of pi's JSONL protocol AND exercises the sketchbook's tool
socket the way the real extension would: on the first page it receives, it
draws two patches over SKETCHBOOK_SOCK (a circle, then a text+curve+arrow
...
def tool_call(cmd):
    ...
    s.connect(os.environ["SKETCHBOOK_SOCK"])
...
for line in sys.stdin:
    ...
    if not responded:
        responded = True
        emit({"type": "tool_execution_start", "toolName": "sketchbook_draw", "args": {}})
        ...
        emit({"type": "tool_execution_start", "toolName": "sketchbook_erase",
              "args": {"id": a.get("id")}})
