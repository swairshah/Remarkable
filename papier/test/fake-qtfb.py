#!/usr/bin/env python3
"""Fake AppLoad/qtfb server for Papier's `make preview` (no tablet).
...
(test/fake-pi.py) wired in via PAPIER_PI_BIN, then scripts a session and
...
class PaperHarness(Harness):
    """The shared harness + Papier's env: fake pi, data dir, tool socket."""

    def launch(self, **env_extra):
        base = dict(PAPIER_PI_BIN=os.path.join(self.here, "fake-pi.py"),
                    PAPIER_DATA_DIR=DATA_DIR,
                    PAPIER_SOCK="/tmp/au.sock")
        base.update(env_extra)
        return super().launch(drop=("PAPIER_OPEN",), **base)
...
def scenario_m1(h, out_png):
    ...
    # -- session A: the book -------------------------------------------------
    s = h.launch(PAPIER_OPEN="demo-paper")
    ...
    # -- session C: resume via settings.json (no PAPIER_OPEN) -----------------
    s = h.launch()
    time.sleep(1.8)
...
def scenario_m2(h, out_png):
    ...
    env = dict(PAPIER_FAKE_SYS="1",
               PAPIER_LEGACY_READER="/tmp/legacy-books",
               PAPIER_LEGACY_NOTEBOOK="/tmp/legacy-nb")
...
def scenario_garamond(h, out_png):
    ...
    s = h.launch(PAPIER_OPEN="nb", PAPIER_FAKE_SYS="1", FAKE_PI_SCRIPT="garamond")
    s.drain(1.8)
...
def scenario_fontflip(h, out_png):
    ...
    s = h.launch(PAPIER_OPEN="nb", PAPIER_FAKE_SYS="1")
    s.drain(1.5)
    pen_tap(s, *TB_TOGGLE)
    s.drain(0.4)
...
def scenario_m3(h, out_png):
    ...
    s = h.launch(PAPIER_OPEN="nb-test", PAPIER_FAKE_SYS="1")
    s.drain(1.8)
...
def scenario_m4(h, out_png):
    ...
    s = h.launch(PAPIER_OPEN="nb-sel", PAPIER_FAKE_SYS="1")
    s.drain(1.8)
...
def scenario_m5_book(h, out_png):
    ...
    s = h.launch(PAPIER_OPEN="demo-paper", PAPIER_FAKE_SYS="1", FAKE_PI_SCRIPT="book")
    s.drain(2.2)
...
def scenario_m5_nb(h, out_png):
    ...
    s = h.launch(PAPIER_OPEN="nb-pi", PAPIER_FAKE_SYS="1", FAKE_PI_SCRIPT="notebook")
    s.drain(1.8)
...
def scenario_agent(h, out_png):
    ...
    with open(agent_md, "w") as f:
        f.write("# papier agent - standing instructions\n\n"
                "- (nothing yet - when the user tells you how they want you to\n"
                "  behave while they read, record it here)\n")

    s = h.launch(PAPIER_FAKE_SYS="1", FAKE_PI_SCRIPT="agent",
                 PAPIER_AGENT_MD=agent_md)
    s.drain(1.8)
...
def scenario_erasemodes(h, out_png):
    ...
    s = h.launch(PAPIER_OPEN="nb-er", PAPIER_FAKE_SYS="1")
    s.drain(1.5)
...
def main():
    ...
    scenario = SCENARIOS[os.environ.get("PAPIER_SCENARIO", "m1")]
