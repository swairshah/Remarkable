...
const markedJs = path.resolve(__dirname, '../web/website/marked.js');
...
test('site builder makes swair.dev root the post index and copies only chosen assets', (t) => {
  ...
  assert.match(writingCss, /--link-color: #6b6b6b/);
  assert.match(writingCss, /--link-color: #8f8b82/);
...
test('website editor is linked in the shared header and supports preview plus save', () => {
  ...
  assert.match(ui, /<script src="\/website\/marked\.js"><\/script>/);
  assert.match(fs.readFileSync(markedJs, 'utf8'), /marked v18\.0\.11/);
  assert.match(ui, /window\.marked\.parse\(markdown/);
  assert.match(ui, /updatePreview\(false\)/);
  assert.match(ui, /exactNow \? 0 : 900/);
...
`);
  ...
  assert.match(prompt, /existing post is authoritative for unchanged material/);
  assert.match(prompt, /paragraph and list structure is part of the writing/);
  assert.match(prompt, /one sentence physically continues across/);
  assert.match(prompt, /enumeration STAYS a list/);
  assert.match(prompt, /Never dissolve it into prose or invent connective wording/);
  ...
  assert.match(prompt, /\[polish\] at the start of a page or section/);
