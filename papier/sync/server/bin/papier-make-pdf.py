#!/usr/bin/env python3
"""Build a searchable PDF from an papier book bundle.

    papier-make-pdf.py <doc-dir> <out.pdf> [title]

Used by the Papier upload service when a book has NO retained source PDF
...
Runs in the papier venv (pymupdf), same as mkbook.py.
"""
import glob
import json
import os
...
def main() -> int:
    ...
    if not pages:
        print(f"papier-make-pdf: no pages in {docdir}", file=sys.stderr)
        return 1
