...
//!
//! Layout (PAPIER_DATA_DIR, default ~/.local/share/papier):
//!
...
pub fn data_dir() -> String {
    if let Ok(d) = std::env::var("PAPIER_DATA_DIR") {
        return d;
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/root".into());
    format!("{home}/.local/share/papier")
}
...
fn legacy_reader_dir() -> String {
    if let Ok(d) = std::env::var("PAPIER_LEGACY_READER") {
        return d;
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/root".into());
...
fn legacy_notebook_dir() -> String {
    if let Ok(d) = std::env::var("PAPIER_LEGACY_NOTEBOOK") {
        return d;
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/root".into());
...
pub fn import_legacy() -> (usize, usize) {
    ...
    if let Ok(rd) = std::fs::read_dir(legacy_reader_dir()) {
        for e in rd.flatten() {
            ...
            if std::fs::rename(e.path(), &dest).is_ok() {
                books += 1;
                println!("papier: imported book '{slug}'");
            }
        }
    }
    ...
    if let Ok(rd) = std::fs::read_dir(&nb_src) {
        ...
        if !nums.is_empty() {
            ...
            println!("papier: imported {pages} notebook pages as 'Notebook (imported)'");
        }
    }
