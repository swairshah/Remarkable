...
//! papier-canvas.ts extension (its page tools) via `-e`. When the user
...
fn session_dir() -> String {
    if let Ok(d) = std::env::var("PAPIER_SESSION_DIR") {
        return d;
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/root".into());
    format!("{home}/.local/share/papier/sessions")
}
...
run a computation), never as a side effect of an ordinary page.";
    ...
    /// Spawn pi in RPC mode with the reader extension. PAPIER_PI_BIN
    /// overrides the binary (the preview harness points it at a fake);
    /// PAPIER_EXT is the extension path (set by takeover.sh);
    ...
    pub fn spawn(sock: &str) -> std::io::Result<Pi> {
        ...
        let agent_md = std::env::var("PAPIER_AGENT_MD")
            .unwrap_or_else(|_| format!("{home}/.local/share/papier/AGENT.md"));
        ...
        if !std::path::Path::new(&agent_md).exists() {
            let _ = std::fs::write(
                &agent_md,
                "# papier agent - standing instructions\n\n\
