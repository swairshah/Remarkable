//! Papier — a pixel-faithful replication of the stock reMarkable UI with
...
pub const APP: libreink_core::app::AppId =
    libreink_core::app::AppId { name: "papier", env_prefix: "PAPIER" };
...
fn agent_md_path() -> String {
    if let Ok(p) = std::env::var("PAPIER_AGENT_MD") {
        return p;
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/root".into());
    format!("{home}/.local/share/papier/AGENT.md")
}
...
/// real pressure (0..4095). Same response as the original, one shade
/// finer: 1.8..3.4 (originally 2.0..3.6).
fn brush_r(pressure: i32) -> f32 {
    1.8 + pressure as f32 / 4096.0 * 1.6
}
...
fn sock_path() -> String {
    std::env::var("PAPIER_SOCK").unwrap_or_else(|_| "/tmp/papier-ctl.sock".into())
}
...
impl App {
    ...
    fn home_rescan(&mut self) {
        ...
        if nv.sig != cur_sig {
            println!("papier: docs changed on disk — refreshing home");
    ...
    fn trigger_sync(&mut self) {
        ...
        let _ = std::process::Command::new("/home/root/bin/papier-sync.sh")
    ...
    fn open_doc(&mut self, id: &str) -> bool {
        match Doc::open(id) {
            Some(d) => {
                println!(
                    "papier: opened '{}' ({} pdf pages, {} entries, at {})",
                    d.title,
                    d.pdf_pages,
                    d.count(),
                    d.current + 1
            ...
            None => {
                println!("papier: could not open doc '{id}'");
                false
            }
        }
    }
        ...
        match dialog {
            ...
            Dialog::ConfirmAddPage => {
                if let Some(0) = dialog_row_at(2, x, y) {
                    ...
                    println!("papier: added blank page at {}", at + 1);
                    self.jump_to(at + 1);
                    self.trigger_sync();
                } else {
    ...
    fn open_agent_page(&mut self) {
        ...
        println!("papier: AGENT.md page opened");
    }
    ...
    fn close_agent_page(&mut self) {
        ...
        println!("papier: AGENT.md page closed");
    }
    ...
    fn send_agent_feedback(&mut self) {
        ...
        match pi.send_image_message(&gray, w as u32, h as u32, &msg, streaming) {
            Ok(()) => {
                ...
                println!("papier: AGENT.md annotations sent to pi");
            }
            Err(e) => println!("papier: agent feedback send failed: {e}"),
        }
    }
    ...
    fn maybe_send_page(&mut self) {
        ...
        match pi.send_image_message(&gray, w as u32, h as u32, &msg, streaming) {
            Ok(()) => {
                ...
                self.pi_alive_at = Instant::now();
                self.set_working(true);
                println!("papier: page {page_no} sent to pi");
            }
            Err(e) => println!("papier: send failed: {e}"),
        }
    }
    ...
    fn handle_pi(&mut self, ev: PiEvent) {
        ...
        match ev {
            ...
            PiEvent::Died(reason) => {
                ...
                println!("papier: pi exited: {reason}");
            }
        }
    }
    ...
    fn check_pi_health(&mut self) {
        ...
        {
            println!(
                "papier: pi silent for {}s mid-turn; respawning",
        ...
        if self.pi.is_none() && self.pi_respawn_at.is_some_and(|at| Instant::now() >= at) {
            ...
            match pi_rpc::spawn(&sock_path()) {
                Ok(p) => {
                    self.pi = Some(p);
                    self.pi_alive_at = Instant::now();
                    println!("papier: pi respawned");
                }
                Err(e) => {
                    println!("papier: pi respawn failed: {e}");
    ...
    fn ipc_underline(&mut self, req: &Value) -> Value {
        ...
        match self.add_patch_at(idx, strokes, Vec::new()) {
            Ok((id, bbox, _)) => {
                println!("papier: underlined '{phrase}' on page {} (#{id})", idx + 1);
                json!({
    ...
    fn ipc_goto(&mut self, req: &Value) -> Value {
        ...
        self.jump_to(idx + 1);
        println!("papier: pi turned to page {}", idx + 1);
        let Screen::Doc(dv) = &self.screen else { return Self::no_doc() };
        json!({
    ...
    fn ipc_insert_note(&mut self, req: &Value) -> Value {
        ...
        println!("papier: pi inserted note page at {}", at + 1);
        let indicator = {
            let Screen::Doc(dv) = &self.screen else { return Self::no_doc() };
...
fn sleep_cycle(
    ...
    'sleeping: loop {
        ...
        if attempts >= 8 {
            println!("papier: suspend never happened ({attempts} tries); waking the page");
            break;
        }
        println!("papier: suspend aborted (EPD discharge timer), retrying");
    }
    println!("papier: waking");
...
fn main() -> std::process::ExitCode {
    ...
    if std::env::var("PAPIER_IMPORT_ONLY").is_ok() {
        let (b, p) = store::import_legacy();
        println!("papier: import done ({b} books, {p} notebook pages)");
    ...
    let mut touchdev = if takeover {
        touch::TouchDevice::open(APP)
            .map_err(|e| eprintln!("papier: no touch device ({e}) — page flips disabled"))
            .ok()
    } else {
        None
    };
    let mut powerdev = if takeover {
        power::PowerButton::open(APP)
            .map_err(|e| eprintln!("papier: no power button ({e})"))
            .ok()
    } else {
        None
    };
     ...
     * PAPIER_AUTO_SLEEP_MIN (minutes), 0 disables. */
    let auto_sleep_min: u64 = std::env::var("PAPIER_AUTO_SLEEP_MIN")
        .ok()
    ...
    let ipc = IpcServer::open(APP, &sock)
        .map_err(|e| eprintln!("papier: tool socket: {e} — pi gets no tools"))
        .ok();
    let pi = match pi_rpc::spawn(&sock) {
        Ok(p) => Some(p),
        Err(e) => {
            eprintln!("papier: could not start pi: {e}");
            None
        }
    };
    let pi_stall = std::env::var("PAPIER_PI_STALL")
        .ok()
    ...
    /* first paint: PAPIER_OPEN (harness), else resume the last doc, else home */
    let open_id = std::env::var("PAPIER_OPEN").ok().unwrap_or(last_doc);
    ...
    while RUNNING.load(Ordering::Relaxed) {
        ...
        if let (Some(limit), Some(p)) = (auto_sleep, powerdev.as_mut()) {
            ...
            {
                println!("papier: idle {auto_sleep_min}min -> auto-sleep");
                sleep_cycle(&mut app, p, &mut pen, &mut touchdev);
                power_grace = Instant::now() + Duration::from_secs(3);
                last_activity = Instant::now();
            }
