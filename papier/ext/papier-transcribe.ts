/**
 * papier-transcribe.ts — image→text context compression for Papier's pi session
 * (device copy of context-management/extensions/transcribe.ts; keep in sync).
 *
 ...
 *
 * Store persists to ~/.local/share/papier/transcriptions.json keyed by image hash.
...
const STORE =
  process.env.PAPIER_TRANSCRIBE_STORE ??
  `${process.env.HOME || "/home/root"}/.local/share/papier/transcriptions.json`;
...
/** Parse a Papier page-pause user message: page number, doc title, content snippet. */
