#!/bin/sh
# alt-ui two-way web sync (tablet side). Event-driven — no timer:
#
#   on edit  (Paper, debounced)  -> both     pull inbound + push mirror
#   on sleep (rm-sync-flush.sh)  -> push     mirror only, radio then rests
#   on wake  (rm-sync-wake.sh)   -> pull     inbound only
#
#   usage: alt-ui-sync.sh [pull|push|both]     (default: both)
#
...
do_push() {
    # mirror the tablet's alt-ui data to the web (true mirror)
