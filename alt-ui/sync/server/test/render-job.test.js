...
test('render jobs return immediately, report progress, and deliver atomically', async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'paper-render-job-'));
  const backup = path.join(root, 'backup');
  const renderer = path.join(root, 'fake-render.js');
  const port = await freePort();
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));

  writeJson(path.join(backup, 'alt-ui', 'docs', 'book', 'meta.json'), { title: 'Book', pages: 2, w: 1404, h: 1872 });
  fs.mkdirSync(path.join(backup, 'alt-ui-sources'), { recursive: true });
  fs.writeFileSync(path.join(backup, 'alt-ui-sources', 'book.pdf'), '%PDF-test');
...
`);
  ...
  const service = spawn(process.execPath, [path.resolve(__dirname, '../bin/alt-ui-upload.js')], {
    env: { ...process.env, ALT_UI_BACKUP: backup, ALT_UI_RENDER: renderer, ALT_UI_PORT: String(port) },
  ...
  assert.deepEqual(JSON.parse(fs.readFileSync(path.join(backup, 'alt-ui-inbound', 'docs', 'book', 'meta.json'))).crop,
    [0.1, 0.1, 0.9, 0.9]);
  assert.equal(fs.readFileSync(path.join(backup, 'alt-ui-inbound', 'docs', 'book', 'thumb.png'), 'utf8'), 'cover');
});
