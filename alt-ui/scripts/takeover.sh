#!/bin/bash
# Launch Paper in full-takeover mode on the reMarkable 2: stop xochitl,
# host the e-ink engine with the bundled rm2fb server (timower/rM2-stuff),
# run Paper against it (instant ink), ALWAYS restore xochitl on exit.
#
# Exit paper: top-edge swipe -> CLOSE, or SIGTERM; the power button
...
echo "paper-takeover: closed ($?), restoring xochitl"
