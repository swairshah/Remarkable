/**
 * paper-metrics.ts — per-turn pi telemetry on the device.
 *
 * Answers "what took how long, and how much context was sent" for every pi
 * call. Loaded alongside paper-canvas.ts via PAPER_EXT (colon-separated).
 *
 * Writes one JSON line per event to $PAPER_METRICS
 * (default ~/.local/share/alt-ui/metrics.jsonl):
 ...
 * Also prints a one-line summary per turn to stderr → /tmp/paper.log.
 *
...
const OUT =
  process.env.PAPER_METRICS ??
  `${process.env.HOME || "/home/root"}/.local/share/alt-ui/metrics.jsonl`;
