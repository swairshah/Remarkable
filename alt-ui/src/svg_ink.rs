...
use crate::hershey;
use crate::ink::{Pt, Stroke, TextRun, AI_GRAY};

/// The face pi writes in. The first three are Hershey single-stroke plotter
/// fonts (drawn as pen strokes that animate + erase like ink); Garamond is
/// the real EB Garamond outline font, rendered typeset (text.rs) and stored
/// as a `TextRun` — richer serif, and it syncs to the web in true Garamond.
...
impl PiFont {
    /// Menu order for the picker.
    pub const ALL: [PiFont; 4] = [PiFont::Script, PiFont::Serif, PiFont::Sans, PiFont::Garamond];

    pub fn key(self) -> &'static str {
        match self {
            PiFont::Sans => "sans",
            PiFont::Script => "script",
            PiFont::Serif => "serif",
            PiFont::Garamond => "garamond",
        }
    }
    pub fn label(self) -> &'static str {
        match self {
            PiFont::Sans => "Sans (plotter)",
            PiFont::Script => "Script (cursive)",
            PiFont::Serif => "Serif (roman)",
            PiFont::Garamond => "Garamond (typeset)",
        }
    }
    pub fn from_key(s: &str) -> Option<PiFont> {
        PiFont::ALL.iter().copied().find(|f| f.key() == s)
    }
    /// Resolve an SVG font-family; None if it names nothing we recognize.
    fn from_family(name: &str) -> Option<PiFont> {
        if name.to_ascii_lowercase().contains("garamond") {
            return Some(PiFont::Garamond);
        }
        hershey::face_from_name(name).map(PiFont::from)
    }
    /// The Hershey face for a stroke font (None for Garamond, which is typeset).
    fn hershey(self) -> Option<hershey::Face> {
        match self {
            PiFont::Sans => Some(hershey::Face::Sans),
            PiFont::Script => Some(hershey::Face::Script),
            PiFont::Serif => Some(hershey::Face::Serif),
            PiFont::Garamond => None,
        }
    }
}

impl From<hershey::Face> for PiFont {
    fn from(f: hershey::Face) -> PiFont {
        match f {
            hershey::Face::Sans => PiFont::Sans,
            hershey::Face::Script => PiFont::Script,
            hershey::Face::Serif => PiFont::Serif,
        }
    }
}
...
pub fn parse(
    ...
    text_scale: f32,
    default_font: PiFont,
) -> Result<(Vec<Stroke>, Vec<TextRun>, Vec<String>), String> {
    ...
    texts.extend(parse_texts(src, default_font));
    ...
    let measures: Vec<Option<Measured>> = texts
        .iter()
        .map(|t| {
            /* Garamond runs are typeset, handled separately below */
            let Some(face) = t.pf.hershey() else { return None };
            ...
            let full_w = runs_width(face, &runs, size);
    ...
    for (t, m) in texts.iter().zip(&measures) {
        ...
        if m.full_w * sc <= m.max_w || is_math {
            ...
            let face = t.pf.hershey().unwrap_or(hershey::Face::Serif);
            emit_runs(&mut out, face, &m.runs, m.x_anchor, m.y, size, &t.anchor);
        } else {
            ...
            let face = t.pf.hershey().unwrap_or(hershey::Face::Serif);
            let ls = wrap_text(face, content, sz, m.max_w.max(sz));
            let short: String = t.content.chars().take(28).collect();
            notes.push(format!(
            ...
            for (i, line) in ls.iter().enumerate() {
                ...
                emit_runs(&mut out, face, &one, m.x_anchor, ly, sz, &t.anchor);
            }
        }
    }

    /* Garamond elements: typeset runs (positioned + anchor-adjusted), kept
     * verbatim as text so the real font renders on device and web. */
    let mut runs_out: Vec<TextRun> = Vec::new();
    for t in &texts {
        if t.pf != PiFont::Garamond {
            continue;
        }
        ...
        let size = (t.size * k * text_scale).clamp(14.0, 160.0);
        let (ax, y) = tf((t.x, t.y));
        let w = crate::text::width(crate::text::Face::Body, size, content) as f32;
        let x = match t.anchor {
            Anchor::Start => ax,
            Anchor::End => ax - w,
            Anchor::Middle => ax - w / 2.0,
        };
        /* skip a run whose whole box is off-page */
        if x > SCREEN_W as f32 + 20.0 || x + w < -20.0 || y < -20.0 || y > SCREEN_H as f32 + 20.0 {
            notes.push("a Garamond line fell off the page and was dropped".into());
            continue;
        }
        runs_out.push(TextRun { x, y, size, gray: AI_GRAY, text: content.to_string() });
    }
...
fn parse_texts(src: &str, default_font: PiFont) -> Vec<TextEl> {
    let mut out = Vec::new();
    let mut rest = src;
    while let Some(a) = rest.find("<text") {
        ...
        let pf = attr(tag, "font-family")
            .and_then(|f| PiFont::from_family(&f))
