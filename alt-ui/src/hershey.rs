...
//!
//! The default face comes from $PAPER_FONT (sans|script|serif, default
...
/// The faces the user can pick as pi's default handwriting, in menu order.
pub const FACES: [Face; 3] = [Face::Script, Face::Serif, Face::Sans];
...
impl Face {
    ...
    pub fn label(self) -> &'static str {
        match self {
            Face::Sans => "Sans (plotter)",
            Face::Script => "Script (cursive)",
            Face::Serif => "Serif (roman)",
        }
    }
    pub fn from_key(s: &str) -> Option<Face> {
        FACES.iter().copied().find(|f| f.key() == s)
    }
}
...
/// Resolve a font name (from $PAPER_FONT or an SVG font-family) to a
...
/// The app-wide default face ($PAPER_FONT, default serif).
pub fn default_face() -> Face {
    std::env::var("PAPER_FONT")
        .ok()
