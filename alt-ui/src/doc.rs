...
impl Doc {
    ...
    pub fn load_raster(&self, pdf_page: usize) -> Option<Vec<u8>> {
        ...
        match png_dec::decode_png_gray(&data) {
            ...
            Ok((w, h, _)) => {
                eprintln!("paper: page {} raster is {w}x{h}, want {SCREEN_W}x{SCREEN_H}", pdf_page + 1);
                None
            }
            Err(e) => {
                eprintln!("paper: page {} png: {e}", pdf_page + 1);
                None
            }
        }
    }
    ...
    pub fn save_state(&mut self) {
        ...
        if std::fs::write(&tmp, serde_json::to_vec(&doc).unwrap_or_default())
            ...
            .is_err()
        {
            eprintln!("paper: could not save {path}");
        }
