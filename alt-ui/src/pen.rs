...
impl Pen {
    ...
    pub fn open() -> Option<Pen> {
        for i in 0..8 {
            ...
            if n > 0 && name.contains("Wacom") {
                println!("paper: direct pen input from /dev/input/event{i} ({})",
        ...
        println!("paper: no Wacom device, inking via AppLoad events");
        None
    }
    ...
    pub fn grab(&self) {
        ...
        if rc != 0 {
            println!("paper: pen EVIOCGRAB failed ({})", std::io::Error::last_os_error());
        }
    }
