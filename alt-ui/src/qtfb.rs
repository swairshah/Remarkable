...
#[derive(Clone, Copy)]
#[allow(dead_code)] /* collab only uses UltraFast; the rest are API */
...
/// (The framebuffer half lives in fb.rs, shared with the rm2fb backend.)
pub struct Socket {
    fd: RawFd,
    /// Raw view of the shm mapping (ptr, byte len), for msync on update.
    /// qemu-user (the preview harness) emulates MAP_SHARED file mappings
    /// privately and only writes them back on msync/munmap — without this
    /// the fake server never sees our pixels. On a real kernel msync of
    /// tmpfs pages is a no-op-cheap syscall.
    shm: Option<(usize, usize)>,
}
...
impl Socket {
    ...
    fn msync(&self) {
        if let Some((ptr, len)) = self.shm {
            unsafe { libc::msync(ptr as *mut libc::c_void, len, libc::MS_SYNC) };
        }
    }
...
pub fn connect() -> io::Result<(Framebuffer, Socket)> {
    ...
    let mut sock = Socket { fd, shm: None }; /* from here on, Drop cleans up on any error */
    ...
    let expected = (SCREEN_W * SCREEN_H * 2) as usize;
    ...
    sock.shm = Some((ptr as usize, init.shm_size));
    let fb = Framebuffer::from_raw(ptr as *mut u16, init.shm_size / 2);
    Ok((fb, sock))
}
