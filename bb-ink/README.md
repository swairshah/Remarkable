...
The first observation includes a full-page image. Later observations include one when the agent board changes, every eighth observation after the first, or when you tap **Nudge**. Otherwise they send the changed region. Both images show the same agent board, so the agent's own drawing cannot trigger a new observation.
...
**Quiet** pauses automatic sharing. **Nudge** asks the agent to inspect the canvas now, including when Quiet is on. **Reply** opens a read-only response view; **Next / canvas** advances longer replies and then returns to your writing. **Threads** opens projects and recent conversations. The native client supports pen erasing and Undo. The desktop companion supports drawing, typing, and Undo.
...
The launcher restores xochitl on exit. Five fingers or a power-button press exits; ten minutes without input also returns to xochitl. Logs are `/tmp/bb-ink.log` and `/tmp/bb-ink-rm2fb.log`.
