...
**Quiet** pauses automatic sharing. **Nudge** asks the agent to inspect the canvas now, including when Quiet is on. Swipe a finger vertically and lift to scroll the same writable document. The page stays still during contact, then commits a deliberate swipe after a 180 ms settle interval. Swipes require at least 80 pixels of mostly vertical motion and a contact duration of 50–900 ms. The pen always writes or erases; pen proximity cancels finger gestures, including a swipe waiting to commit. The native input reader rejects multiple or very broad contacts and latches rejection until every contact is lifted, so a remaining palm contact cannot become a new finger. Stationary pen proximity is checked directly, in addition to the 1500 ms recent-pen guard. See [input.rs](src/input.rs), [touch_input.rs](src/touch_input.rs), and [pen_contact.rs](src/pen_contact.rs). **More ↓** scrolls farther down a response and **Top ↑** returns to the beginning. Handwriting uses document coordinates and stays anchored when you scroll or reopen a conversation. **Threads** opens projects and recent conversations. The native client supports pen erasing and Undo. The desktop companion supports drawing, typing, and Undo.
...
SVG reply text is literal, including underscores, braces, dollar signs, backslashes, and indentation. The default reply lettering is an original curved-stroke pencil alphabet, with content-stable shape, baseline, pressure, and graphite-grain variation. Plain prose uses the same lettering and measured wrapping. `font-family="pencil"`, `"handwritten"`, `"script"`, and generic `"sans"` select it; explicit `"Inter"` keeps printed text and `"monospace"` uses Google Sans Code. Garamond and explicit `"plotter script"` remain available. Non-alphabet symbols fall back to embedded Inter. [pencil.rs](src/pencil.rs) owns the trajectories, rasterization, cache, and layout metrics; [pencil-lettering.json](fixtures/pencil-lettering.json) is its renderable specimen. This is a reference-inspired implementation, not the identified original font from the video. Math uses Unicode or explicitly positioned text rather than implicit TeX conversion. [svg_text.rs](src/svg_text.rs) owns this behavior; [font-options.json](fixtures/font-options.json) is the renderable font specimen. Observation images are prepared in a background worker, preserving writing and erasing that arrive during capture; see [observation.rs](src/observation.rs). Physical palm behavior still requires an on-device writing check.
