...
- An **Example todos** page in the left sidebar that adds, completes, and
  removes todos.
- A `bb ink` command that does the same from a terminal.
...
The bundled skill tells an agent to read the list with `bb ink list`, add
one todo at a time with `bb ink add`, and close finished work with
`bb ink done`.
