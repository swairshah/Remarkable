#!/bin/bash
# Launch reader in full-takeover mode on the reMarkable 2: stop xochitl,
# host the e-ink engine with the bundled rm2fb server (timower/rM2-stuff),
# run reader against it (instant ink), ALWAYS restore xochitl on exit.
#
# Exit reader: top-edge swipe -> CLOSE, or SIGTERM; the power button
...
# READER_EXT points pi at the reader-tools extension shipped next to the
# binary; the app passes it to pi with -e.
# READER_FONT is the default face for pi's writing: serif (formal roman),
...
HOME=/home/root PATH="/home/root/bin:/home/root/opt/node/bin:/usr/bin:/bin:/usr/sbin:/sbin" \
    READER_EXT="$HERE/reader-canvas.ts" \
    READER_FONT="${READER_FONT:-serif}" \
    "$HERE/reader" >>/tmp/reader.log 2>&1
echo "reader-takeover: closed ($?), restoring xochitl"
