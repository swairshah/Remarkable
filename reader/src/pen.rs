...
/// kernel keeps writing 16-byte records. Shared with touch.rs / power.rs.
#[repr(C)]
#[derive(Clone, Copy, Default)]
pub(crate) struct RawInputEvent {
    pub sec: u32,
    pub usec: u32,
    pub kind: u16, /* EV_* ("type" in the kernel struct) */
    pub code: u16,
    pub value: i32,
}

pub(crate) const EVIOCGRAB: libc::c_int = 0x40044590u32 as libc::c_int;
...
impl Pen {
    ...
    pub fn open() -> Option<Pen> {
        for i in 0..8 {
            ...
            let mut name = [0u8; 64];
            let n = unsafe { libc::ioctl(fd, eviocgname(64) as _, name.as_mut_ptr()) };
            let name = String::from_utf8_lossy(&name[..n.max(0) as usize]);
            if n > 0 && name.contains("Wacom") {
                println!("reader: direct pen input from /dev/input/event{i} ({})",
        ...
        println!("reader: no Wacom device, inking via AppLoad events");
        None
    }
    ...
    /// EVIOCGRAB the digitizer (takeover mode): we're the only reader, so
    /// nothing else replays our strokes if xochitl ever comes back early.
    /// Best-effort; the grab is released by the kernel when the fd closes.
    pub fn grab(&self) {
        let rc = unsafe { libc::ioctl(self.fd, EVIOCGRAB as _, 1i32) };
        if rc != 0 {
            println!("reader: pen EVIOCGRAB failed ({})", std::io::Error::last_os_error());
        }
    }
        ...
        loop {
            ...
            for ev in &evs[..n as usize / std::mem::size_of::<RawInputEvent>()] {
                match (ev.kind, ev.code) {
                    ...
                    (EV_SYN, _) => {
                        self.sx = self.wy * crate::fb::SCREEN_W / WACOM_Y_MAX;
                        self.sy =
                            crate::fb::SCREEN_H - self.wx * crate::fb::SCREEN_H / WACOM_X_MAX;
