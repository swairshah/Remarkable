//! The tool socket: how pi's reader_draw / reader_underline / reader_view ...
...
//! ($READER_SOCK) speaking one JSON object per line, request/response.
...
impl IpcServer {
    pub fn open(path: &str) -> io::Result<IpcServer> {
        ...
        set_nonblock(fd);
        println!("reader: tool socket at {path}");
    ...
    pub fn read_conn(&mut self, fd: RawFd) -> Vec<Value> {
        ...
        while let Some(pos) = conn.buf.iter().position(|&b| b == b'\n') {
            ...
            match serde_json::from_slice::<Value>(line) {
                Ok(v) => out.push(v),
                Err(e) => eprintln!("reader: bad ipc json: {e}"),
            }
        }
        self.reap();
        out
