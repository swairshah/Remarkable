 ...
 * `-e <this file>` and READER_SOCK pointing at its unix tool socket.
 * These tools speak one JSON object per line over that socket:
 *
 *   reader_draw        {cmd:"draw", svg, page?}            -> {ok, id, bbox}
 *   reader_underline   {cmd:"underline", phrase, page?}    -> {ok, id, matches}
 *   reader_erase       {cmd:"erase", id, page?}            -> {ok}
 *   reader_view        {cmd:"view", page?}                 -> {ok, png_base64,..}
 *   reader_goto        {cmd:"goto", page}                  -> {ok, layout}
 *   reader_insert_note {cmd:"insert_note", after_page?}    -> {ok, page}
 *   reader_page_text   {cmd:"page_text", from, to?}        -> {ok, text}
 *
 ...
 * messages use.
 *
 * When READER_SOCK is absent (a normal pi session), nothing registers —
...
const SOCK = process.env.READER_SOCK ?? "";
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      ...
      "To underline printed text use reader_underline instead — it is exact. " +
        ...
        return {
          content: [{
            ...
            text: `Drawn on page ${r.page} as patch #${r.id}${bbox}. ` +
              `Use reader_erase with id ${r.id} to remove it.${notes}${layout}`,
          }],
  ...
  pi.registerTool({
    name: "reader_underline",
    label: "reader: underline a printed phrase",
    description:
    ...
    async execute(_id: string, params: any) {
      try {
        ...
        return textResult(
          ...
          `Use reader_erase with id ${r.id} to remove it.`,
        );
      } catch (e: any) {
  ...
  pi.registerTool({
    name: "reader_erase",
    label: "reader: erase one of your patches",
    description:
      "Erase a patch you previously made (reader_draw or reader_underline), by its id. The " +
  ...
  pi.registerTool({
    ...
    description:
      ...
      "plain text prefer reader_page_text, it is much cheaper.",
    parameters: {
      type: "object",
      properties: {
        page: PAGE_PARAM,
  ...
  pi.registerTool({
    name: "reader_insert_note",
    label: "reader: insert a blank note page",
    description:
      ...
      "allows; then write on it with reader_draw {page: N} using the returned page number, " +
      ...
      try {
        ...
        return r.ok
          ? textResult(
              ...
              `Draw on it with reader_draw {page: ${r.page}}.`,
            )
  ...
  pi.registerTool({
    ...
    label: "reader: read pages as text",
    description:
      ...
      "in the range are listed but contain only handwriting (use reader_view to see them).",
    parameters: {
      type: "object",
      properties: {
