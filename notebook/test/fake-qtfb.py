#!/usr/bin/env python3
"""Notebook's `make preview` scenario (no tablet).
...
(NOTEBOOK_PI_BIN), handwrite, wait for the pause trigger -> the fake pi
...
class NotebookHarness(Harness):
    def launch(self, **env_extra):
        base = dict(NOTEBOOK_PI_BIN=os.path.join(self.here, "fake-pi.py"),
...
def main():
    ...
    try:
        ...
        # handwrite a sine-ish squiggle mid-page ("the user's writing")
        s.pen(PEN_PRESS, 160, 420)
        for i in range(1, 80):
            s.pen(PEN_UPDATE, 160 + i * 9, 420 + int(80 * math.sin(i / 5)))
        s.pen(PEN_RELEASE, 160 + 80 * 9, 420)
