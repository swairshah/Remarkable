...
impl Pen {
    ...
    pub fn open() -> Option<Pen> {
        for i in 0..8 {
            ...
            if n > 0 && name.contains("Wacom") {
                println!("notebook: direct pen input from /dev/input/event{i} ({})",
        ...
        println!("notebook: no Wacom device, inking via AppLoad events");
        None
    }
    ...
    pub fn grab(&self) {
        ...
        if rc != 0 {
            println!("notebook: pen EVIOCGRAB failed ({})", std::io::Error::last_os_error());
        }
    }
