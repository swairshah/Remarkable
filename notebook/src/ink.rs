...
//!
//! Persistence: one JSON file per page (ints, coords x10) in the notebook's
//! data dir. Small, dependency-free, debuggable with jq.
...
impl Page {
    ...
    /// Re-render a region from the vectors: white, user ink, then patches
    /// (darkest-wins makes the order mostly moot). Returns whether the
    /// caller should prefer the 16-level waveform — always false now that
    /// screen ink is uniformly black (DU renders it crisply).
    pub fn render_region(&self, fb: &mut Framebuffer, r: Rect) -> bool {
    ...
    pub fn render_full(&self, fb: &mut Framebuffer) {
        self.render_region(fb, Rect { x0: 0, y0: 0, x1: SCREEN_W - 1, y1: SCREEN_H - 1 });
    }

    /// Render the page into a grayscale snapshot at 1/`div` scale for pi:
    /// user ink black, AI ink gray, so the model can tell whose is whose.
