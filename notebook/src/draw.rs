...
impl Framebuffer {
    ...
    pub fn set_clip(&mut self, y0: i32, y1: i32) {
        ...
        self.clip_y1 = y1.min(SCREEN_H);
    }

    pub fn clear_clip(&mut self) {
        self.clip_y0 = 0;
        self.clip_y1 = SCREEN_H;
    }

    pub fn px(&mut self, x: i32, y: i32, c: u16) {
        if (0..SCREEN_W).contains(&x) && y >= self.clip_y0 && y < self.clip_y1 {
            self.pixels()[(y * SCREEN_W + x) as usize] = c;
        }
    }
    ...
    pub fn fill_rect(&mut self, x: i32, y: i32, w: i32, h: i32, c: u16) {
        ...
        let px = self.pixels();
        for row in y0..y1 {
            let base = (row * SCREEN_W) as usize;
    ...
    /// (which is `target = 0`). Used for subtle grey chrome.
