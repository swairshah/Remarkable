...
/// (the TTF cousin of hershey::fold). Everything the fonts do cover —
/// Greek, ∑ ∏ ∫ ∞ √ ≤ ≈ arrows, super/subscripts — passes through
/// untouched and renders via face_for. Idempotent; borrows when clean.
...
/// Whether the face actually has a glyph for `c` (math symbol fallbacks).
pub fn has_glyph(f: Face, c: char) -> bool {
    font(f).lookup_glyph_index(c) != 0
}
...
/// blending glyphs toward black. Used to place `<text>` labels inside a
...
pub fn draw_gray(buf: &mut [u8], bw: i32, bh: i32, x: i32, y_top: i32, f: Face, px: f32, s: &str) {
    draw_gray_level(buf, bw, bh, x, y_top, f, px, s, 0);
}

/// Like `draw_gray`, but glyphs darken toward `level` (0=black) rather than
/// full black, and it's darkest-wins so nothing gets lighter. Used to place
/// pi's typeset ink into the snapshot at AI_GRAY so the model can tell its
/// own writing apart. Scales via a caller-chosen `px`, so it also serves the
/// downscaled page snapshot.
pub fn draw_gray_level(buf: &mut [u8], bw: i32, bh: i32, x: i32, y_top: i32, f: Face, px: f32, s: &str, level: u8) {
            ...
            if m.width > 0 && m.height > 0 {
                ...
                for row in 0..m.height as i32 {
                    ...
                    for col in 0..m.width as i32 {
                        ...
                        /* blend the existing pixel toward `level` by coverage */
                        let cur = buf[idx] as u32;
                        let v = (cur * (255 - a) + level as u32 * a) / 255;
                        if (v as u8) < buf[idx] {
                            buf[idx] = v as u8;
                        }
                    }
...
pub fn wrap(f: Face, px: f32, max_w: i32, text: &str) -> Vec<String> {
    ...
    let text = fold(text); /* so wrap widths match what draw renders */
