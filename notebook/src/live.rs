...
//!
//! Transport: $NOTEBOOK_LIVE_CMD (preview harness), or ssh with the tablet's
//! sync key to the VM's ingest script, which feeds notebook-live-relay and
//! comes out of https://remarkable.exe.xyz/notebook/ as an SSE overlay.
//!
...
fn default_cmd() -> String {
    "exec /usr/bin/ssh -y -i /home/root/.ssh/id_sync_dropbear_ed25519 \
     exedev@remarkable.exe.xyz /home/exedev/bin/notebook-live-ingest.sh"
        .into()
}
...
impl Live {
    ...
    pub fn toggle(&mut self, page: usize) -> bool {
        if self.enabled {
            ...
            self.enabled = false;
            self.retry_at = None;
            println!("notebook: live stream off");
        } else {
            self.enabled = true;
            self.connect(page);
            println!("notebook: live stream on");
        }
        self.enabled
    }
    ...
    fn connect(&mut self, page: usize) {
        self.disconnect();
        let cmd = std::env::var("NOTEBOOK_LIVE_CMD").unwrap_or_else(|_| default_cmd());
            ...
            .spawn()
        {
            Ok(mut child) => {
                ...
                println!("notebook: live stream connecting");
            }
            Err(e) => {
                println!("notebook: live spawn failed: {e}");
    ...
    fn push_line(&mut self, line: String) {
        ...
        if self.buf.len() + line.len() + 1 > BUF_CAP {
            ...
            println!("notebook: live buffer overflow, reconnecting");
            self.disconnect();
            self.retry_at = Some(Instant::now() + RETRY_AFTER);
            return;
        }
    ...
    pub fn tick(&mut self, page: usize) {
        ...
        if let Some(c) = self.child.as_mut() {
            if c.try_wait().ok().flatten().is_some() {
                println!("notebook: live transport exited; retrying soon");
                self.disconnect();
                self.retry_at = Some(Instant::now() + RETRY_AFTER);
            }
        }
        ...
        if let Err(e) = self.flush() {
            println!("notebook: live write failed ({e}); retrying soon");
            self.disconnect();
            self.retry_at = Some(Instant::now() + RETRY_AFTER);
        }
    }
