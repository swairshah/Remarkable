#!/bin/bash
# Launch notebook in full-takeover mode on the reMarkable 2: stop xochitl,
# host the e-ink engine with the bundled rm2fb server (timower/rM2-stuff),
# run notebook against it (instant ink), ALWAYS restore xochitl on exit.
#
# Exit notebook: top-edge swipe -> CLOSE, or SIGTERM; the power button
...
# NOTEBOOK_EXT points pi at the canvas-tools extension shipped next to the
# binary; the app passes it to pi with -e.
# NOTEBOOK_FONT is the default face for pi's writing: serif (formal roman),
...
HOME=/home/root PATH="/home/root/bin:/home/root/opt/node/bin:/usr/bin:/bin:/usr/sbin:/sbin" \
    NOTEBOOK_EXT="$HERE/notebook-canvas.ts" \
    NOTEBOOK_FONT="${NOTEBOOK_FONT:-serif}" \
    "$HERE/notebook" >>/tmp/notebook.log 2>&1
echo "notebook-takeover: closed ($?), restoring xochitl"
