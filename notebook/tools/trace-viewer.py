#!/usr/bin/env python3
"""Render notebook's pi session traces (.jsonl) into one self-contained HTML.

The debugging view this exists for: every notebook_draw call is shown as an
...
Usage:
    ...
    ... --log notebook.log                     # include the device app log
...
def render_device_log(path, out):
    ...
    out.append(
        "<div class='card meta'><div class='role'>device log — /tmp/notebook.log "
...
def main():
    ...
    doc = (f"<!doctype html><meta charset='utf-8'><title>notebook · pi traces</title>"
           f"<style>{CSS}</style>"
