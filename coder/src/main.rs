//! coder — read codebases as diagrams, sketch feedback, get PRs; on the
//! reMarkable 2.
//!
//! The sidebar lists PROJECTS — git repositories cloned on the exe.dev VM —
//! and each project is a stack of paper pages. A background pi agent draws
//! each repo's architecture on its pages (boxes, arrows, plotter-font
//! notes, animated in stroke by stroke like a ghost hand); the user asks
//! questions and requests changes in ink, and pi answers by drawing —
//! or by editing the repo over ssh and opening a pull request. Its
//! drawings are tracked as patches it can later erase or replace via its
//! tools (see ipc.rs / coder-canvas.ts).
//!
...
//!   ipc.rs      unix-socket server for pi's coder_* tools
...
pub const APP: libreink_core::app::AppId =
    libreink_core::app::AppId { name: "coder", env_prefix: "CODER" };
...
use ink::{Codebook, Page, Pt, Rect, RenderExt, Stroke};
use ipc::IpcServer;
 ...
 * CODER_PI_STALL (seconds) overrides, mainly for the preview harness. */
fn pi_stall() -> Duration {
    Duration::from_secs(
        std::env::var("CODER_PI_STALL").ok().and_then(|v| v.parse().ok()).unwrap_or(180),
    )
}
...
enum SbRow {
    Project(usize), /* index into Sb.projects — tap to switch */
    More,           /* "..." overflow marker (inert) */
    Agent,
    Docs, /* pi's SUMMARY.md per project, typeset */
    #[allow(dead_code)] /* out of the menu until the VM has a coder relay */
...
/// Projects at the top (capped so the fixed rows always fit), then chrome.
fn build_sb_rows(n_projects: usize) -> Vec<SbRow> {
    let usable = FB_H - SB_LIST_Y0 - SB_CLOSE_H - 2 * SB_CLOSE_MARGIN;
    let max_rows = (usable / SB_ROW_H) as usize;
    let fixed = 6usize; /* Agent..Refresh + the divider row's slack */
    let cap = max_rows.saturating_sub(fixed).max(1);
    let mut rows = Vec::new();
    for i in 0..n_projects.min(cap) {
        rows.push(SbRow::Project(i));
    }
    if n_projects > cap {
        rows.push(SbRow::More);
    }
    /* Live stays out of the menu until the VM has a coder relay
     * (live.rs still speaks the protocol; see notebook's ingest) */
    rows.extend([
        SbRow::Agent,
        SbRow::Docs,
        SbRow::Quiet,
...
fn settings_path() -> String {
    ...
    format!("{home}/.local/share/coder/settings.json")
}

/// (text_scale, quiet, pi_font, eraser, project) from settings.json; all
/// optional in the file. pi_font None = no override ($CODER_FONT fallback);
/// project = the slug that was on screen last run.
fn load_settings() -> (f32, bool, Option<hershey::Face>, EraserMode, String) {
    ...
    let proj = v
        ...
        .filter(|s| project::exists(s))
        .unwrap_or(project::NOTES_SLUG)
        .to_string();
    (scale, quiet, font, eraser, proj)
}

fn save_settings(scale: f32, quiet: bool, font: hershey::Face, eraser: EraserMode, project: &str) {
    let p = settings_path();
...
struct Sb {
    projects: Vec<project::Project>, /* scanned when the panel opened */
    rows: Vec<SbRow>,
}
...
fn agent_md_path() -> String {
    if let Ok(p) = std::env::var("CODER_AGENT_MD") {
        return p;
    }
    let home = std::env::var("HOME").unwrap_or_else(|_| "/home/root".into());
    format!("{home}/.local/share/coder/AGENT.md")
}
...
/* the DOCS browser: SUMMARY.md list, or one summary paginated */
const LIB_LIST_Y0: i32 = 130;
const LIB_ROW_H: i32 = 112;

enum LibView {
    List { items: Vec<project::DocItem> },
...
fn sock_path() -> String {
    std::env::var("CODER_SOCK").unwrap_or_else(|_| "/tmp/coder-ctl.sock".into())
}
...
struct App {
    ...
    /* the AGENT.md page, when open (replaces the coder view) */
...
impl App {
    ...
    fn indicator_rect(&self) -> Rect {
        Rect { x0: FB_W / 2 - 180, y0: FB_H - 56, x1: FB_W / 2 + 180, y1: FB_H - 10 }
    }

    fn show_page_indicator(&mut self) {
        /* the 5x7 chrome font is caps-only territory; slugs are lowercase */
        let mut tag = self.nb.slug.to_uppercase();
        tag.truncate(9);
        let label = format!("{}  {} / {}", tag, self.nb.current + 1, self.nb.count);
    ...
    fn show_sidebar(&mut self) {
        ...
        let projects = project::scan();
        let rows = build_sb_rows(projects.len());
        self.sidebar = Some(Sb { projects, rows });
    ...
    fn paint_sidebar(&mut self) {
        ...
        self.fb.text(28, 24, "CODER", 4, BLACK);
        let sub = format!("{}  -  page {} of {}", self.nb.slug, self.nb.current + 1, self.nb.count);
        self.fb.text(28, 62, &sub, 2, draw::GRAY);
        self.fb.fill_rect(0, 92, SB_W - 3, 2, BLACK);

        {
            let rows = sb.rows.clone();
            let projects: Vec<(String, String, usize)> = sb
                .projects
                .iter()
                .map(|p| (p.slug.clone(), p.name.clone(), p.pages))
                .collect();
            for (i, row) in rows.iter().enumerate() {
                let y = SB_LIST_Y0 + i as i32 * SB_ROW_H;
                let (label, current) = match row {
                    SbRow::Project(pi) => {
                        let Some((slug, name, pages)) = projects.get(*pi) else { continue };
                        let mut label = if slug == project::NOTES_SLUG {
                            "NOTES".to_string()
                        } else {
                            name.to_uppercase()
                        };
                        /* row width budget: ~ (SB_W - 36 - 60) / 18 px per char at size 3 */
                        while text_width(&label, 3) > SB_W - 36 - 76 && label.len() > 3 {
                            label.pop();
                        }
                        if *pages > 1 {
                            label.push_str(&format!(" ({pages})"));
                        }
                        let cur = self.nb.slug == *slug
                            && self.agent_page.is_none()
                            && self.lib_view.is_none();
                        (label, cur)
                    }
                    SbRow::More => ("...".to_string(), false),
                    SbRow::Agent => ("INSTRUCTIONS".to_string(), self.agent_page.is_some()),
                    SbRow::Docs => (
                        format!("DOCS ({})", project::docs().len()),
    ...
    /// Leave any menu view and land on coder page `p` (0-based).
    ...
    fn sidebar_press(&mut self, x: i32, y: i32) {
        ...
        if in_rect(x, y, SB_CLOSE_MARGIN, cy, SB_W - 2 * SB_CLOSE_MARGIN, SB_CLOSE_H) {
            println!("coder: close (sidebar)");
            RUNNING.store(false, Ordering::Relaxed);
            return;
        }
        let idx = (y - SB_LIST_Y0) / SB_ROW_H;
        let Some(row) = (idx >= 0)
            .then(|| self.sidebar.as_ref().and_then(|s| s.rows.get(idx as usize)))
            .flatten()
            .copied()
        else {
        ...
        match row {
            ...
            SbRow::PiFont => {
                /* coder's toggle order (serif first); parse() takes the
                ...
                println!("coder: pi font -> {}", self.pi_font.key());
                self.paint_sidebar(); /* stays open for repeated taps */
            }
            SbRow::Quiet => {
                self.quiet = !self.quiet;
                save_settings(self.text_scale, self.quiet, self.pi_font, self.eraser, &self.nb.slug);
                println!("coder: pi {}", if self.quiet { "quiet" } else { "auto" });
    ...
    fn switch_project(&mut self, slug: &str) {
        ...
        /* per-page transient state dies with the old project */
        self.cur_stroke = None;
        self.anim.clear();
    ...
    fn open_library(&mut self) {
        ...
        self.idle_at = None;
        self.indicator_until = None;
        self.lib_view = Some(LibView::List { items: project::docs() });
        self.render_library(true);
        println!("coder: library opened");
    }
    ...
    fn render_library(&mut self, flash: bool) {
        ...
        match &self.lib_view {
            Some(LibView::List { items }) => {
                let head = format!("DOCS  ({} projects)", items.len());
                self.fb.text(24, 18, &head, 3, BLACK);
                self.fb.text(
                    24,
                    52,
                    "pi's living notes per repo - tap to read - swipe left to return",
                    2,
                    draw::GRAY,
                );
                ...
                let rows: Vec<(String, String)> = items
                    .iter()
                    .map(|it| {
                        (it.title.clone(), format!("{}  -  {} KB  -  {}", it.date, it.kb, it.slug))
                    })
                    .collect();
                if rows.is_empty() {
                    text::draw_line(
                        ...
                        text::Face::Body,
                        34.0,
                        "Empty. pi writes a SUMMARY.md per cloned project - ask it to clone a repo.",
                    );
                }
    ...
    fn library_press(&mut self, _x: i32, y: i32) {
        ...
        let Some(content) = project::read_summary(&it.slug) else { return };
    ...
    fn open_agent_page(&mut self) {
        ...
        println!("coder: AGENT.md page opened");
    }
    ...
    fn close_agent_page(&mut self) {
        ...
        /* a pending coder change resumes its pause countdown */
        if self.page_changed {
            self.idle_at = Some(Instant::now() + IDLE_DELAY);
        }
        self.show_page_indicator();
        println!("coder: AGENT.md page closed");
    }
    ...
    fn send_agent_feedback(&mut self) {
        ...
        match pi.send_image_message(&gray, w as u32, h as u32, &msg, streaming) {
            Ok(()) => {
                ...
                println!("coder: AGENT.md annotations sent to pi");
            }
            Err(e) => println!("coder: agent feedback send failed: {e}"),
        }
    }
    ...
    /// coder_view — only the conversation history is retired.
    fn reset_session(&mut self) {
        ...
        println!("coder: session reset ({archived} transcript(s) archived); fresh pi incoming");
    }
    ...
    fn do_undo(&mut self) {
        ...
        println!("coder: undo ({} left)", self.undo.len());
    }
    ...
    fn do_redo(&mut self) {
        ...
        println!("coder: redo ({} left)", self.redo.len());
    }
    ...
    fn apply_drag(&mut self, sel: Selection, dx: i32, dy: i32) {
        ...
        println!("coder: selection moved by ({dx},{dy})");
    }
    ...
    fn flip(&mut self, delta: i32) {
        ...
        /* on the AGENT.md page: forward returns to the coder, further
         * back does nothing */
        if self.agent_page.is_some() {
            if delta > 0 {
        ...
        println!("coder: page {} / {}", self.nb.current + 1, self.nb.count);
    }
    ...
    /// the coder page, or the AGENT.md page's annotation layer.
    ...
    fn wipe_raster_at(&mut self, x: i32, y: i32) -> bool {
        ...
        self.restore_chrome_over(r);
        println!("coder: raster #{} wiped on page {}", rl.id, self.nb.current + 1);
        true
    }
    ...
    fn finish_rub_loop(&mut self) {
        ...
        if changed {
            ...
            self.nb.save_current();
            self.deghost_at = Some(Instant::now() + Duration::from_millis(1100));
            println!("coder: region erase on page {}", self.nb.current + 1);
        }
        if let Some(d) = dirty {
    ...
    fn maybe_send_page(&mut self) {
        ...
        match pi.send_page(&gray, w as u32, h as u32, &context, &patches, &layout, streaming) {
            Ok(()) => {
                ...
                self.pi_alive_at = Some(Instant::now());
                println!(
                    "coder: {} page {} sent to pi",
                    self.nb.slug,
                    self.nb.current + 1
                );
            }
            Err(e) => println!("coder: send failed: {e}"),
        }
    }
    ...
    fn handle_pi(&mut self, ev: PiEvent) {
        match ev {
            ...
            PiEvent::Notice(n) => println!("coder: pi {n}"),
            PiEvent::End => {
                self.streaming = false;
                self.set_working(false);
                self.live.status("idle");
    ...
    /// The sidebar's data, for pi: every project + which one is on screen.
    fn ipc_projects(&mut self) -> Value {
        let list: Vec<Value> = project::scan()
            .iter()
            .map(|p| {
                json!({
                    "slug": p.slug,
                    ...
                    "url": p.url,
                    "summary": p.summary,
                    "pages": if p.slug == self.nb.slug { self.nb.count } else { p.pages },
                    "on_screen": p.slug == self.nb.slug,
                })
            })
            .collect();
        json!({
            "ok": true,
            "projects": list,
            "current": self.nb.slug,
            "page": self.nb.current + 1,
    ...
    fn ipc_place(&mut self, req: &Value) -> Value {
        ...
        if idx == self.nb.current {
            ...
            println!("coder: raster #{id} {dw}x{dh} placed at ({px},{py}) on page {}", idx + 1);
    ...
    fn ipc_erase_ink(&mut self, req: &Value) -> Value {
        ...
        if idx == self.nb.current {
            ...
            println!("coder: erased {removed} user strokes in rect on page {}", idx + 1);
    ...
    fn ipc_draw(&mut self, req: &Value) -> Value {
        ...
        if idx == self.nb.current {
            ...
            self.nb.save_current();
            self.last_activity_page = idx;
            println!("coder: patch #{id} on page {} ({n_strokes} strokes)", idx + 1);
            json!({
             ...
             * page == count+1 appends a fresh blank page (multi-page docs) */
            if idx > self.nb.count {
                return json!({ "ok": false, "error": format!("no page {} (project '{}' has {}; the next new page would be {})", idx + 1, self.nb.slug, self.nb.count, self.nb.count + 1) });
            }
            let appended = idx == self.nb.count;
            if appended {
                self.nb.append_page();
            }
            let path = self.nb.page_path(idx);
    ...
    fn ipc_goto(&mut self, req: &Value) -> Value {
        let want_project = req["project"].as_str().filter(|s| !s.is_empty());
        let want_page = req["page"].as_u64().filter(|&p| p >= 1);
        if want_project.is_none() && want_page.is_none() {
            return json!({ "ok": false, "error": "pass 'project' (slug) and/or 'page' (1-based)" });
        }
        if self.agent_page.is_some() || self.sidebar.is_some() || self.lib_view.is_some() {
            return json!({ "ok": false, "error": "the user is in a menu/instructions/docs view; not changing the screen. They will find your work in the sidebar" });
        }
        if self.cur_stroke.is_some()
        ...
        if let Some(p) = want_page {
            let idx = (p as usize - 1).min(self.nb.count.saturating_sub(1));
            if p as usize > self.nb.count {
                return json!({ "ok": false, "error": format!("no page {} (project '{}' has {}; coder_draw with page={} appends a new one)", p, self.nb.slug, self.nb.count, self.nb.count + 1) });
            }
            if idx != self.nb.current {
        ...
        if self.streaming && self.pi.is_some() {
            if let Some(at) = self.pi_alive_at {
                if at.elapsed() >= self.pi_stall {
                    println!(
                        "coder: pi silent for {}s mid-turn; restarting it",
                        at.elapsed().as_secs()
                    );
                ...
                if Instant::now() >= at {
                    match pi_rpc::spawn(&self.sock) {
                        Ok(p) => {
                            ...
                            println!("coder: pi respawned (session continued)");
                        ...
                        Err(e) => {
                            println!("coder: pi respawn failed: {e}; retrying in 30s");
...
/// The pause message's project line: which project, which repo, which page.
fn project_context(nb: &Codebook) -> String {
    let page = format!("page {} of {}", nb.current + 1, nb.count);
    if nb.slug == project::NOTES_SLUG {
        return format!("Coder project 'notes' (the scratch notes pad - no repo), {page}");
    }
    let p = project::get(&nb.slug);
    let mut what = if p.url.is_empty() { String::new() } else { format!("{} - ", p.url) };
    what.push_str(&format!("clone at {}/{} on the VM", pi_rpc::vm_dir(), nb.slug));
    let sum = if p.summary.is_empty() { String::new() } else { format!("; {}", p.summary) };
    format!("Coder project '{}' ({what}{sum}), {page}", nb.slug)
}
...
fn sleep_cycle(
    ...
    'sleeping: loop {
        ...
        if attempts >= 8 {
            println!("coder: suspend never happened ({attempts} tries); waking the page");
            break;
        }
        println!("coder: suspend aborted (EPD discharge timer), retrying");
    }
    println!("coder: waking");
...
fn main() -> std::process::ExitCode {
    ...
    let ipc = IpcServer::open(APP, &sock)
        .map_err(|e| eprintln!("coder: tool socket: {e} — pi gets no drawing tools"))
        .ok();
    let pi = match pi_rpc::spawn(&sock) {
        Ok(p) => Some(p),
        Err(e) => {
            eprintln!("coder: could not start pi: {e}");
            None
        }
    };
    ...
    let mut touchdev = if takeover {
        touch::TouchDevice::open(APP)
            .map_err(|e| eprintln!("coder: no touch device ({e}) — page flips disabled"))
            .ok()
    } else {
        None
    };
    let mut powerdev = if takeover {
        power::PowerButton::open(APP)
            .map_err(|e| eprintln!("coder: no power button ({e})"))
            .ok()
    } else {
        None
    };
     ...
     * CODER_AUTO_SLEEP_MIN (minutes), 0 disables. */
    let auto_sleep_min: u64 = std::env::var("CODER_AUTO_SLEEP_MIN")
        .ok()
    ...
    let now = Instant::now();
    project::ensure_notes();
    let (text_scale, quiet, saved_font, eraser, start_project) = load_settings();
    let pi_font = saved_font.unwrap_or_else(|| hershey::default_face(APP));
    let mut app = App {
        fb,
        disp,
        pi,
        nb: Codebook::open(&start_project),
        ipc,
    ...
    while RUNNING.load(Ordering::Relaxed) {
        ...
        if let (Some(limit), Some(p)) = (auto_sleep, powerdev.as_mut()) {
            ...
            {
                println!("coder: idle {auto_sleep_min}min -> auto-sleep");
                sleep_cycle(&mut app, p, &mut pen, &mut touchdev);
                power_grace = Instant::now() + Duration::from_secs(3);
                last_activity = Instant::now();
            }
